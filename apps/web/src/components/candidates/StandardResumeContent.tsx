...
const StandardResumeContent: React.FC<CandidateProps> = ({ candidate }) => {
  if (!candidate) return (
    <div className="py-32 flex flex-col items-center justify-center">
       <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={<span className="text-white/30 text-[12px]">暂无简历数据</span>} />
    </div>
  );
  ...
  const skills: string[] = candidate.parsedTags?.skills || [];
    ...
    <div className="p-10 flex flex-col gap-12 bg-bg-base text-text-main">
      ...
      <section>
        ...
        <div className="grid grid-cols-4 gap-y-8 bg-bg-elevated/20 border border-border-subtle p-8 rounded-2xl">
          ...
          <InfoItem label="工作年限" value={candidate.totalYears ? `${candidate.totalYears}年` : '--'} />
          ...
          <div className="flex flex-col gap-10">
            {workExp.map((exp: any, index: number) => {
              const contentArray = Array.isArray(exp.content)
                ? exp.content
                : (typeof exp.description === 'string' ? [exp.description] : (typeof exp.content === 'string' ? [exp.content] : []));
                ...
                <div key={index} className="relative pl-8 border-l border-border-subtle group/item">
                  ...
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-[14px] font-medium text-text-main/80">{exp.position || '--'}</span>
                    </div>
                    <span className="text-[12px] font-medium text-text-sub/50 font-mono">{exp.duration || '--'}</span>
                  </div>
                  ...
                  {contentArray.length > 0 && (
                    <div className="flex flex-col gap-2.5 px-1">
                      ...
                      {contentArray.map((item: string, i: number) => (
                        <div key={i} className="flex gap-2.5 items-start text-[13px] leading-relaxed text-text-main/70">
              ...
              <div key={index} className="relative pl-8 border-l border-border-subtle group/item">
                ...
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-3 text-[15px] font-medium">
                    <span className="text-text-main hover:text-brand-primary cursor-pointer transition-colors">{edu.school || '--'}</span>
                    <span className="opacity-20">·</span>
                    <span className="text-text-main/80">{edu.degreeLevel || edu.degree || '--'}</span>
                    <span className="opacity-20">·</span>
                    <span className="text-text-main/60">{edu.major || '--'}</span>
                  </div>
                  <span className="text-[12px] font-medium text-text-sub/50 font-mono">{edu.duration || '--'}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <section>
          ...
          {skills.length === 0 ? (
            ...
            <div className="bg-bg-elevated/20 border border-border-subtle p-6 rounded-lg flex flex-wrap gap-2">
              {skills.map(skill => (
                <Tag key={skill} className="m-0 bg-brand-primary/10 border-none text-brand-primary px-3 py-1 rounded-md text-[12px] font-bold hover:bg-brand-primary/20 transition-all cursor-default">
                  {skill}
                </Tag>
              ))}
            </div>
          ...
          {projList.length === 0 ? (
            ...
            <div className="flex flex-col gap-4">
              {projList.map((proj: any, i: number) => (
                <div key={i} className="bg-bg-elevated/20 border border-border-subtle p-5 rounded-lg">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-[14px] font-bold text-text-main">{proj.projectName || '--'}</span>
                    {proj.role && <span className="text-[12px] text-brand-primary">{proj.role}</span>}
                  </div>
                  {proj.duration && <span className="text-[11px] text-text-sub/40 font-mono">{proj.duration}</span>}
                  {proj.description && <p className="text-[12px] text-text-sub/80 mt-2 m-0 leading-relaxed">{proj.description}</p>}
                </div>
              ))}
            </div>
          )}
      ...
      <div className="mt-12 text-center border-t border-border-subtle pt-12">
        <p className="text-[10px] font-black uppercase tracking-[0.8em] text-text-sub/10">End of Standard Structured Profile</p>
      </div>
    </div>
  );
};
