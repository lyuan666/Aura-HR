    ...
    <Card variant="borderless" className="h-full rounded-2xl shadow-sm border border-slate-50">
      ...
      <List
        ...
        dataSource={data.length > 0 ? data : [
          { title: '资深后端开发', company: '阿里巴巴', urgency: 'urgent', count: 3 },
          { title: 'AI 算法专家', company: '月之暗面', urgency: 'high', count: 1 },
          { title: '产品总监', company: '美团', urgency: 'high', count: 2 },
        ]}
        renderItem={(item: any) => (
          <List.Item className="px-0 border-b-slate-50 group cursor-pointer hover:bg-slate-50/50 transition-colors rounded-lg px-2">
            <div className="flex items-center w-full">
              ...
              <div className="text-right">
                ...
                <div className="text-[10px] text-slate-300">待推荐</div>
              </div>
            </div>
          </List.Item>
        )}
  ...
  return (
    <Card variant="borderless" className="h-full rounded-2xl shadow-sm bg-gradient-to-br from-white to-indigo-50/30">
      ...
      <div className="space-y-4">
        {(data.length > 0 ? data : [
          { name: '字节跳动', status: 'signed', lastContact: '2天前', nextAction: '约见招聘负责人' },
          { name: '蚂蚁金服', status: 'negotiating', lastContact: '5小时前', nextAction: '确认需求细节' },
        ]).map((client: any) => (
...
export const PipelineFunnelWidget = ({ data = [] }: { data?: any[] }) => {
  ...
  const defaultData = [
    { name: '已推荐', value: 120, fill: '#6366f1' },
    { name: '约面中', value: 45, fill: '#818cf8' },
    { name: '评估中', value: 22, fill: '#a5b4fc' },
    { name: 'Offer', value: 8, fill: '#c7d2fe' },
    { name: '已入职', value: 3, fill: '#e0e7ff' },
  ];

  const chartData = (data && data.length > 0 && data.some(d => d.value > 0)) ? data : defaultData;
    ...
    <Card variant="borderless" className="h-full rounded-2xl shadow-sm" styles={{ body: { padding: 20 } }}>
      ...
      <div className="mt-4 flex justify-between border-t border-slate-50 pt-4">
         <div className="text-center">
            ...
            <div className="text-lg font-black text-slate-700">32.1%</div>
         </div>
         <div className="text-center">
            <div className="text-xs text-slate-400 mb-1">入职达成</div>
            <div className="text-lg font-black text-slate-700">85%</div>
         </div>
      </div>
    </Card>
  );
