...
import { StateMachine, RECOMMENDATION_TRANSITIONS } from '../../common/utils/state-machine';
...
export class RecommendationService {
  ...
  async createRecommendation(candidateId: string, jobId: string, tenantId?: string) {
    // 检查是否已存在
    const existing = await this.recommendationRepo.findOne({
      where: { candidateId, jobPositionId: jobId, ...(tenantId ? { tenantId } : {}) },
    });
  ...
  async createInterviewReport(recommendationId: string, interviewText: string, tenantId?: string) {
    const rec = await this.recommendationRepo.findOne({ 
      where: { id: recommendationId, ...(tenantId ? { tenantId } : {}) } 
    });
    if (!rec) throw new NotFoundException('Recommendation not found');
  ...
  async updateStatus(id: string, status: string, tenantId?: string) {
    ...
    const updated = await this.recommendationRepo.findOne({ where: { id } });
    return this.dehydrate(updated!);
  }

  async updateSchedule(id: string, date: Date, tenantId?: string) {
    const rec = await this.recommendationRepo.findOne({ 
      where: { id, ...(tenantId ? { tenantId } : {}) } 
    });
    if (!rec) throw new NotFoundException('Recommendation not found');
