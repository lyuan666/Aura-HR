...
import { StateMachine, CONTRACT_TRANSITIONS } from '../../common/utils/state-machine';
...
export class ContractService {
  ...
  async create(dto: any, tenantId?: string) {
    const contract = this.contractRepo.create({ ...dto, tenantId, status: 'draft' });
  ...
  async uploadAndCreate(file: Express.Multer.File, body: any, tenantId?: string) {
    const safeName = file.originalname.replace(/[/\\]/g, '_');
    const fileKey = `contracts/${tenantId || 'global'}/${uuid()}-${safeName}`;
  ...
  getTemplates() {
    return [
      { id: 'labor', name: '劳动合同模板', description: '标准劳动合同，适用于全职员工录用', fileType: 'PDF' },
      { id: 'service', name: '服务协议模板', description: '猎头服务合作协议，适用于客户签约', fileType: 'PDF' },
      { id: 'nda', name: '保密协议模板', description: '保密及竞业限制协议', fileType: 'PDF' },
      { id: 'recommendation', name: '候选人推荐函模板', description: '正式候选人推荐信函格式', fileType: 'PDF' },
    ];
  }
}
